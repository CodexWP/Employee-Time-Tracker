                            ...
                            if($projects)
                            {
                                foreach ($projects as $project)
                                {
                                    ...
                                    echo '<tr>
                                            ...
                                            <td  class="text-center"><a target="_blank" href="'.$this->Url->build("/projects/single/").$project->project_id.'"><i title="Edit" role="button" class="fa fa-edit text-themecolor" aria-hidden="true"></i> Edit</a> <a href="#" class="deleteproject text-danger"><i title="Delete" role="button" class=" fa fa-trash text-danger m-l-10" aria-hidden="true"></i> Delete</a></td>
                                          </tr>';
                                }
                            }
                            else
...
<script type="application/javascript">
    $("document").ready(function(){
        ...
        $("#members-table").on("click",'.btn-project-member-delete', function(){
            ...
            $tr = $(this).parents('tr');
            $.ajax({
                url: ajaxurl + "?op=deleteprojectmember&project_id="+pid+"&userid="+uid,
                type: 'get',
                processData: false,
                contentType: false,
                dataType:'json',
        ...
        $("#btn-add-project-member").click(function(){
            var pid = $("input[name='project_id']").attr('pid');
            var uid = $("select[name='members-list']").val();
            var mname = $("select[name='members-list'] option:selected").text();
            if(uid==''){alert('Please select a member first.');return;}
            $.ajax({
                url: ajaxurl + "?op=addprojectmember&project_id="+pid+"&userid="+uid,
                type: 'get',
                processData: false,
                contentType: false,
                dataType:'json',
                ...
                success:function(resp){
                    ...
                    {
                        $html =  '<tr>' +
                                '<td><i class="fa fa-user" aria-hidden="true"></i> ' + mname + '</td><td><h3 class="text-danger cursor-pointer "><i uid="' + uid + '" class="fa fa-trash-o btn-project-member-delete" aria-hidden="true"></i></h3></td>' +
                                '</tr>';
                        $(".no-members").hide();
                ...
                success:function(resp){
                    if(resp.status=='success') {
                        ...
                        for (i = 0; i < members.length; i++) {
                            $html = $html +
                                '<tr>' +
                                '<td><i class="fa fa-user" aria-hidden="true"></i> ' + members[i].name + '</td><td><h3 class="text-danger cursor-pointer"><i uid="' + members[i].uid + '" class="fa fa-trash-o btn-project-member-delete" aria-hidden="true"></i></h3></td>' +
                                '</tr>';
                            $("#members-table").html($html);
                        }
                        if ($html == '') {
                            $html = '<tr class="no-members"><td>No members are found.</td></tr>';
                            $("#members-table").html($html);
                        }
                    }
                    else{
